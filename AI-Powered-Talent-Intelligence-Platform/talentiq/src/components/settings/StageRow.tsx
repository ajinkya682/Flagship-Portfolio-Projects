...
export default function StageRow({ id, name, isDefault, color }: StageRowProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    >
      ...
      <div className="flex-1 flex items-center gap-[16px]">
        <input 
          type="text" 
          defaultValue={name}
          className="flex-1 h-[36px] rounded-md border border-transparent hover:border-neutral-200 focus:border-primary-500 px-[12px] font-body text-[14px] font-semibold text-neutral-900 focus:outline-none focus:ring-1 focus:ring-primary-500 transition-colors"
        />
        
        <span className={`px-[8px] py-[2px] rounded-full text-[11px] font-bold uppercase tracking-wider ${
          isDefault ? 'bg-neutral-100 text-neutral-600' : 'bg-primary-50 text-primary-700'
        }`}>
      ...
      <div className="flex items-center gap-[16px]">
        <div className="flex items-center gap-[8px]">
          ...
          <select 
            className="w-[24px] opacity-0 absolute cursor-pointer"
            defaultValue={color}
          >
            {/* Color options mock */}
        ...
        <button 
          ...
          title={isDefault ? "Default stages cannot be deleted" : "Delete stage"}
        >
          <Trash2 size={18} />
        </button>
      </div>
