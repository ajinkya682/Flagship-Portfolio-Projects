...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!forceGenerate) {
      const existingSummary = await AISummary.findOne({ weekOf: currentMonday });
      if (existingSummary) {
    ...
    const stagesCount: Record<string, number> = {
      'Applied': 0, 'Screening': 0, 'Interview': 0, 'Assessment': 0, 'Offer': 0, 'Hired': 0, 'Rejected': 0
    };
    ...
    applications.forEach((app: any) => {
      ...
      if (app.timeline && app.timeline.some((t: any) => t.type === 'offer_accepted')) {
        totalOffersAccepted++;
      }
    ...
    const avgAiScore = scoredCandidates > 0 ? Math.round(sumAiScore / scoredCandidates) : 0;
    ...
    const pipelineHealth = Math.min(100, Math.max(0, 70 + (totalHired / Math.max(1, totalCandidates)) * 100));

    // 2. Cross-job Matching Logic
    // Find stalled/rejected candidates to match with open jobs
    const stalledApps = applications.filter((app: any) => app.stage === 'Rejected' || app.daysInStage > 14);
    ...
    for (const app of stalledApps) {
      ...
      for (const job of jobs) {
        if (job._id.toString() === app.job._id.toString()) continue; // Skip their original job

        const jobReqs = (job.requirements || []).join(' ').toLowerCase();
        
        // Count how many candidate skills match job requirements
        const matchedSkills = candidateSkills.filter((s: string) => jobReqs.includes(s.toLowerCase()));
        ...
        if (matchScore >= 90) {
          crossMatches.push({
            ...
            originalJobId: app.job._id,
            originalJobTitle: app.job.title,
