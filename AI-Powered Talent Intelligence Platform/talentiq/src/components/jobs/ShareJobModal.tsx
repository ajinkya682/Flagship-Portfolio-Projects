...
export default function ShareJobModal({ job, companySlug, onClose }: ShareJobModalProps) {
  ...
  return (
    <Dialog.Root open={true} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-50 bg-neutral-900/40 backdrop-blur-sm animate-in fade-in duration-200" />
        <Dialog.Content className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] z-50 bg-white rounded-[24px] shadow-xl w-[90vw] max-w-[500px] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
          
          {/* Header */}
          <div className="px-[24px] py-[20px] border-b border-neutral-100 flex items-center justify-between bg-neutral-50/50">
            <Dialog.Title className="font-display text-[18px] font-bold text-neutral-900">Share Job Post</Dialog.Title>
            <Dialog.Close asChild>
              <button className="p-[6px] text-neutral-400 hover:text-neutral-700 hover:bg-neutral-100 rounded-full transition-colors">
                <X size={20} />
              </button>
            </Dialog.Close>
          </div>
